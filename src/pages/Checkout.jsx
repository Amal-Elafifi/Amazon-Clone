...
export default function Checkout() {
  ...
  const isLoadingOrders = useSelector(
    (store) => store.ordersReducer.isLoadingOrders
  );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (paymentMethod == "cash") {
        ...
        dispatch(
          handleOnlinePayment({ cartId: cartInfo?.cartId, values: values })
        );
      }
    },
      ...
      <div className="py-20 grid place-items-center">
        ...
        <div className="max-w-[587px] max-h-[817px]  border rounded-xl py-[51px] px-[37px] space-y-4">
          ...
          formik.touched.shippingAddress?.details ? (
            ...
            <div>
              <label htmlFor="floating_phone" className="text-xl font-semibold">
                Your Phone
              </label>
              <input
                type="tel"
        ...
        {formik.errors.shippingAddress?.phone &&
          ...
          <button
            ...
            type="submit"
            className="text-white bg-footerbg  font-bold px-5 py-2.5  rounded-lg text-base transition-all duration-300   text-center active:scale-90"
          >
