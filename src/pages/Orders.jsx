...
export default function Orders() {
  ...
  return (
    <>
      {orders?.length==0?<div className='text-center font-Lato mt-5'>
          ...
          <Link to="/" className='bg-[red] text-white font-bold px-4 py-3 rounded-lg hover:text-white border border-[red] hover:scale-110 active:scale-95'>Continue Shopping</Link>
        </div>:<section className='font-Lato mt-5'>
        <div className="text-center  flex items-center justify-center gap-3 relative before:absolute before:bottom-0 before:left-[50%] before:w-24 before:rounded-lg before:h-1 before:top-full before:-translate-x-1/2 before:bg-[red]">
        ...
        {orders?.map((order,index)=><div key={order.id} className="order my-5 p-1">
          <header className='mb-2'>
            <div className=" flex flex-row justify-center sm:gap-5 w-fit mx-auto ">
              ...
              <div className="flex flex-col text-center gap-5">
                ...
                <p className='text-lg font-bold'>
                  Total Order Price : <span className='font-light text-[red]'>{order.totalOrderPrice}</span>
                </p>
          ...
          <div className='rounded-lg overflow-hidden border-[1px] w-full md:w-9/12 mx-auto table-auto border-b-[0px] border-[red]'>
          <table className='w-full text-center font-bold table-fixed  '>
            <thead className='items-center  bg-[red] text-white text-[10px] md:text-sm'>
               <tr>
                <th></th>
                <th>Product</th>
                <th>Price</th>
            ...
            <tbody className='text-[10px] md:text-lg'>
                {order.cartItems.map((product)=>
                    <tr key={product._id} className='border-b border-[red]'>
